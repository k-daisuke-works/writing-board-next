...
## 本番提供開始前チェックリスト（現在はテスト運用・利用者ゼロ）

法人（社会福祉士会）へのサービス提供開始前に揃えるもの。順序もこの通りが望ましい:

1. **東京リージョン移行**（上記優先度1）— 個人情報が入る前に済ませるのが最も低リスク
2. 移行後、`app/privacy/page.tsx` の保管場所記載と `docs/legal/service-agreement-draft.md` 別紙のデータ所在を更新
3. **サービス利用契約の締結** — `docs/legal/service-agreement-draft.md` をベースに、弁護士等のレビュー（特に第4条の委託構成・漏えい時の役割分担、第8条の免責）を経て利用団体と締結
4. 利用団体側の運用確認: 入会時の規約・ポリシー説明、管理者によるアカウント管理・退会時削除の運用
5. 本番開始（この時点から実個人情報の取り扱い開始）
