...
- **有効化に必要な作業**: ①マイグレーション `20260710_instagram.sql` の適用、②会の IG をプロアカウント化して長期トークンを発行し `instagram_accounts` に INSERT（手順はマイグレーションファイルのコメント）。未登録の間は `/sns` が「未連携」表示になるだけで安全
