...
提供形態は**無償・実験提供**（2026-07-10 ユーザー確認。契約締結の予定なし）。

> **2026-07-18 再監査:** 実利用開始前に解消すべき法務・セキュリティ課題が見つかった。下の旧チェックリストだけで利用開始可とは判断しない。`docs/legal/LEGAL_AUDIT_BACKLOG.md` の **Gate A** を完了し、DMを使う場合は **Gate B** も完了すること。実在する相談援助対象者・患者・利用者の情報は、それまで入力禁止とする。
...
4. ~~利用開始~~ → **2026-07-18再監査により保留**。上記Gate A（DMを使う場合はGate Bも）の完了後に再判定
...
- **電気通信事業法の該当性確認（2026-07-12 legal-check 指摘で優先度引き上げ）**: DM＝「他人の通信の媒介」は有償・無償を問わず該当し得るため、**無償の現段階でも一度専門家に届出要否を確認するのが安全**（複数団体への反復継続提供という構造のため）。「当事者の一方の同意による管理者開示」構成の妥当性も併せて確認事項に
- **DM関連の設計課題（legal-check 指摘）**: ①アカウント削除でDMスレッド全体がCASCADE削除される（相手方の発言・報告済みスレッドの証拠も消える）→ writing_data 同様の「匿名化して残す」方式への変更を検討（現状はポリシー7条に実態を明記して整合済み）。②管理者の是正手段が閲覧のみ → 「該当ペアの強制ブロック」機能の追加を検討（現状は既存のアカウント凍結で対応・規約4の2条に明記済み）
